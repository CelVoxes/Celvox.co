...
source("modality_pipeline.R", local = TRUE)
source("predict.R", local = TRUE)
source("analysis.R", local = TRUE)
source("harmonize.R", local = TRUE)
...
harmonize_data <- local({
    function(req) {
        modality <- parse_requested_modality(req)
        if (!is_available_modality(modality)) {
            return(planned_modality_response(modality, "harmonize"))
        }
        dispatch_modality_stage(modality, "harmonize", req = req)
    }
})
...
tsne <- local({
    function(req) {
        ...
        # Explore mode: with no uploaded/harmonized session, embed the reference
        # cohort alone (shared cache) instead of erroring on a missing session.
        tsne_result <- if (session_has_embedding(cache_dir)) {
            run_tsne(cache_dir)
        } else {
            reference_tsne(disease_selection)
        }
...
mutation_tsne <- function(req) {
    ...
    # Load t-SNE results
    tsne_result <- embedding_for_request(req)
...
bridge_predict <- local({
    function(req) {
        ...
        bundle_path <- resolve_asset(bridge_assets$bundle_candidates)
        meta_path <- resolve_asset(bridge_assets$meta_candidates)
        ckpt_path <- resolve_asset(bridge_assets$ckpt_candidates)
        lr_path <- resolve_asset(bridge_assets$lr_candidates)
...
#* Uses unboxedJSON (auto_unbox = TRUE): scalar fields serialize as scalars
#* (id "qc", not ["qc"]) and as_json_array() lists stay arrays. The default
#* `json` serializer is auto_unbox = FALSE, which double-wraps every length-1
#* value and breaks the frontend's catalog parsing (capability modalities,
#* cohort ids, provides) — see computeViewAvailability.
#* @get /catalog
#* @serializer unboxedJSON
function(req) {
...
function(req) {
    modality <- parse_requested_modality(req)
    if (!is_available_modality(modality)) {
        return(planned_modality_response(modality, "qc"))
    }
    dispatch_modality_stage(modality, "qc", req = req)
}
